...
const ParticleCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const setCanvasSize = () => {
      ...
      defaultFaceRadiusX = clamp(width * (width < 760 ? 0.26 : 0.14), 96, height * 0.23);
      defaultFaceRadiusY = defaultFaceRadiusX * 1.38;
    ...
    const init = () => {
      ...
      activeClusters = [];
      faceApparition = null;
      const initialClusters = Math.floor((cols * rows) / 240);
    ...
    const getRoleColor = (role: number) => {
      if (role === ROLE_FACE) return '#bfdbfe';
      if (role === ROLE_FEATURE) return '#f8fafc';
      if (role === ROLE_VOICE) return '#22d3ee';
      if (role === ROLE_SCAN) return '#34d399';
    ...
    const drawAmbientPass = (time: number, faceState: FaceState | null) => {
      ...
      if (faceState) {
        ...
        aura.addColorStop(0, `rgba(14, 165, 233, ${0.13 * faceState.intensity})`);
        aura.addColorStop(0.45, `rgba(20, 184, 166, ${0.07 * faceState.intensity})`);
      ...
      ctx.globalAlpha = faceState ? 0.09 : 0.13;
      ctx.fillStyle = '#22d3ee';
      ctx.fillRect(0, scanY, width, 1);
      ctx.globalAlpha = faceState ? 0.04 : 0.065;
    ...
    const getAnimatedFaceDot = (col: number, row: number, time: number, faceState: FaceState | null) => {
      ...
      if (leftPupil || rightPupil || nostrils || philtrum) {
        return { role: ROLE_FEATURE, opacity: (0.84 + noise * 0.08) * faceState.intensity };
      }

      if (leftEye || rightEye || leftBrow || rightBrow || noseBridge || noseSide || noseTip || upperLip || lowerLip || mouthCrease || chin || jawLine) {
        return { role: ROLE_FEATURE, opacity: (0.54 + noise * 0.26 + mouthPulse * 0.08) * faceState.intensity };
      }

      if (leftEyeSocket || rightEyeSocket) {
        return { role: ROLE_FACE, opacity: (0.24 + noise * 0.18) * faceState.intensity };
      }
      ...
      if (cheekShadow || templeShadow || beardShadow || ear || neck || shoulders) {
        return { role: ROLE_FACE, opacity: (0.16 + noise * 0.17) * faceState.intensity };
      }

      return { role: ROLE_FACE, opacity: (0.09 + noise * 0.13) * faceState.intensity };
    };
      ...
      for (let i = 0; i < numDots; i++) {
        ...
        const rowGlitch =
          inFaceField && Math.sin(frameTime * 5.6 + row * 0.42) > 0.985 && row % 3 === 0
            ...
            : 0;
        const x = col * GAP + (GAP - DOT_SIZE) * 0.5 + rowGlitch;
        const y = row * GAP + (GAP - DOT_SIZE) * 0.5;
        ...
        if (animatedFaceDot) {
          ...
          opacity = Math.max(opacity, animatedFaceDot.opacity * clusterMultiplier);
        }

        const scanBoost = clamp(1 - Math.abs(y - scanY) / 52, 0, 1);
    ...
    const animate = () => {
      ...
      if (activeClusters.length < Math.floor((cols * rows) / 220) && Math.random() < 0.22) {
        spawnCluster();
      }
